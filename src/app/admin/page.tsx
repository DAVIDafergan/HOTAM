        ...
        <Table>
          ...
          <TableBody>
            {paginatedData.map((scribe: any) => {
              const scribeOrders = (orders || []).filter((o: any) => o.sellerId === scribe.id && o.status === 'completed');
              ...
              return (
                <TableRow key={scribe.id} className="hover:bg-muted/10 transition-colors border-muted/20">
                  ...
                  <TableCell>
                    <Badge variant="outline" className={cn("text-[8px] font-black uppercase px-2 py-0.5 rounded-full", scribe.hasScribeCertificate === 'valid' ? 'bg-emerald-50 text-emerald-600 border-emerald-200' : 'bg-orange-50 text-orange-600 border-orange-200')}>
                      {certLabels[scribe.hasScribeCertificate] || scribe.hasScribeCertificate || 'לא צוין'}
                    </Badge>
                  </TableCell>
        ...
        <Table>
          ...
          <TableBody>
            {paginatedData.map((cust: any) => {
              const custOrdersCount = (orders || []).filter((o: any) => o.buyerId === cust.id && o.status === 'completed').length;
              return (
