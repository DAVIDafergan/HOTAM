  ...
  if (!window.__googleMapsPlacesPromise) {
    window.__googleMapsPlacesPromise = new Promise<void>((resolve, reject) => {
      ...
      if (existing) {
        ...
        existing.addEventListener('error', () => reject(new Error('Google Maps script loading failed (check API key, billing, and network) / טעינת Google Maps נכשלה.')));
        return;
      }

      const script = document.createElement('script');
      script.src = `https://maps.googleapis.com/maps/api/js?key=${googleMapsApiKey}&loading=async&libraries=places&language=he&region=IL`;
      script.async = true;
      script.defer = true;
      script.dataset.googleMapsPlaces = 'true';
      script.onload = () => resolve();
      script.onerror = () => reject(new Error('Google Maps script loading failed (check API key, billing, and network) / טעינת Google Maps נכשלה.'));
