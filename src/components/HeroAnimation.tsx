  ...
  return (
    <section className="relative min-h-screen w-full flex items-center justify-center overflow-hidden bg-[#FDFCF0] py-12">
      <div className="absolute inset-0 z-0">
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-[#FDFCF0]/20 via-[#FDFCF0]/80 to-[#FDFCF0]" />
      </div>
        ...
        <div className="max-w-4xl w-full space-y-12 flex flex-col items-center text-center">
          ...
          <div className="space-y-4 animate-fade-in">
            <motion.div 
              ...
              className="inline-block px-5 py-2 bg-primary/5 border border-primary/10 rounded-full text-primary font-black text-[10px] uppercase tracking-[0.3em] mb-2"
            >
          ...
          <div className="w-full bg-white/70 backdrop-blur-3xl border border-white/60 rounded-[3rem] p-6 md:p-10 shadow-premium relative">
            
            <div className="flex flex-col items-center mb-8 gap-3">
              <div className="flex justify-center gap-3">
                {[1, 2, 3].map((s) => (
                  <div key={s} className={cn("h-2 rounded-full transition-all duration-700", step === s ? "w-12 bg-primary shadow-sm" : "w-3 bg-primary/10")} />
                ))}
              </div>
              ...
              {step === 3 && (
                <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6 text-right">
                  ...
                  <div className="flex justify-center pt-10">
                    <Button onClick={handleFinalSearch} className="w-full md:w-auto bg-accent text-primary hover:bg-accent/90 rounded-full px-20 h-16 font-black text-lg uppercase tracking-[0.2em] shadow-2xl gap-4 hover:scale-105 transition-all group active:scale-95">
...
function CategoryCard({ icon, label, onClick }: any) {
  return (
    <button 
      onClick={onClick} 
      className="group flex flex-col items-center gap-3 p-5 rounded-[2.5rem] bg-white border-2 border-transparent shadow-sm hover:shadow-2xl hover:border-accent/30 hover:-translate-y-2 transition-all duration-500 w-full"
    >
      <div className="w-12 h-12 md:w-16 md:h-12 rounded-2xl bg-primary/5 flex items-center justify-center text-primary group-hover:bg-accent group-hover:text-primary transition-all duration-500">
        {React.cloneElement(icon, { className: "w-6 h-6 md:w-7 md:h-7" })}
      </div>
      <span className="font-black text-primary text-[11px] md:text-xs tracking-tight">{label}</span>
    </button>
  );
}
...
function CustomTile({ value, label, active, compact = false }: any) {
  return (
    <Label className={cn(
      "flex items-center justify-between rounded-2xl border-2 transition-all cursor-pointer group px-5",
      compact ? "py-3" : "py-4",
      active ? "border-primary bg-primary/5 shadow-md scale-[1.02]" : "border-primary/5 bg-white/40 hover:bg-white hover:border-primary/10"
    )}>
      <RadioGroupItem value={value} className="hidden" />
      <span className={cn("font-bold text-primary transition-colors", compact ? "text-[10px]" : "text-sm", active ? "opacity-100" : "opacity-60")}>{label}</span>
      <div className={cn("w-3.5 h-3.5 rounded-full border-2 transition-all", active ? "bg-primary border-primary scale-110 shadow-sm" : "border-primary/10 group-hover:border-primary/30")} />
    </Label>
  );
}
